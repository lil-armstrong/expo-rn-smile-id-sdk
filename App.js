...
export default function App() {
    ...
    return Platform.OS === 'ios' ? (
      ...
      <Picker
        ...
        onValueChange={(itemValue, itemIndex) => {
          ...
          console.log(`Got ${itemValue} from ${itemIndex}`);
        }}
      >
  ...
  const processResponse = async (result: any) => {
    ...
    if (result.job_success) {
      ...
      if (result.result.ResultCode && result.result.ResultText) {
        console.error(result.result.ResultCode, result.result.ResultText)
        alertDialog(
          'Job failed',
          `Response with result code ${result.result.ResultCode} and result text ${result.result.ResultText}`
        );
  ...
  return (
    <View style={styles.container}>
      {modal()}
      {loading && <Text style={styles.buttonText}>Progress : {progress}</Text>}
